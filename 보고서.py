...
with col1_bottom:
    ...
    # (리테일 월별 요약 테이블 및 그래프 로직은 기존과 동일)
    # --- 월별/분기별 요약 계산 ---
...
with col2_bottom:
    if show_monthly_summary:
        ...
        corp_df_data = {'7월': [july_pipeline, july_apply, july_distribute], '8월': [august_pipeline, august_apply, august_distribute]}
        ...
        html_corp = html_corp.replace('<td>진척률</td>', '<td style="background-color: #e0f7fa;">진척률</td>').replace(f'<td>{formatted_progress_corp}</td>', f'<td>{formatted_progress_corp}</td>')
